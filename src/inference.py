...
def infer(model, testloader, class_params, output_path):
    ...
    with torch.no_grad():
        for images, _ in tqdm.tqdm(testloader, total=len(testloader)):
            ...
            for mask in masks:
                ...
                mask, num_predict = post_process(sigmoid(mask), class_params[image_id % 4][0],
    ...
    sub.to_csv(f'submission_{round(non_empty/all, 3)}.csv', columns=['Image_Label', 'EncodedPixels'], index=False)
...
if __name__ == "__main__":
    ...
    test_dataset = CloudDataset(df=df_test, path=test_data_path, img_ids=test_ids,
